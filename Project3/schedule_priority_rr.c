...
void add(char *name, int priority, int burst, int tid) {
  ...
  if (curr->next && curr->next->task->priority == priority) {
    while (curr->next && curr->next->task->name < name &&
...
// invoke the scheduler which uses round robin algorithm to schedule the tasks.
// If a task is finished, then it is removed from the list. If a task is not
// finished, then it is moved to the end of the list.
void schedule() {
